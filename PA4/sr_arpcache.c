...
void sr_arpcache_handle_arpreq(struct sr_instance *sr, struct sr_arpreq *req)
{
    ...
    */
    ...
    if (difftime(curtime, req->sent) > 0.5)
    {
        ...
        if (req->times_sent >= 5) /* ip 는 있는데 arp 응답 없는경우 */
        {
            /**************** fill in code here *****************/
            ...
            for (pck = req->packets; pck != NULL; pck = pck->next)
            {
                ...
                /* validation */
                if (ntohs(i_hdr0->ip_len) < ICMP_DATA_SIZE)
                    continue;
                ...
                ict3_hdr->icmp_type = 3; /* destination unreachable */
                ict3_hdr->icmp_code = 1; /* host unreachable */
                ict3_hdr->unused = 0;
                ict3_hdr->next_mtu = 0;
                ...
                if (rtentry != NULL) /* 있을거긴 하지만 혹시모르니까 */
                {
                    /* Ethernet header */
                    ...
                    {
                        /* queue */
                        sr_arpcache_queuereq(cache, rtentry->gw.s_addr, buf, len, rtentry->interface);
                    }
                }
            ...
            /* done */
            sr_arpreq_destroy(cache, req); /* 5번 실패한 요청 삭제 */
        ...
        else
        {
            ...
            ifc = sr_get_interface(sr, req->packets->iface); /* 나가는 interface */
            a_hdr->ar_hrd = htons(arp_hrd_ethernet); /* Ethernet */ 
            a_hdr->ar_pro = htons(ethertype_ip); /*IPv4*/ 
            a_hdr->ar_hln = ETHER_ADDR_LEN;
            a_hdr->ar_pln = 4; 
            a_hdr->ar_op = htons(arp_op_request); /* request */
            ...
            /* Ethernet header */
            e_hdr = (struct sr_ethernet_hdr *)buf;
            memcpy(e_hdr->ether_shost, ifc->addr, ETHER_ADDR_LEN);  /* 내 MAC */
            memset(e_hdr->ether_dhost, 0xFF, ETHER_ADDR_LEN); /* broadcast */ 
            e_hdr->ether_type = htons(ethertype_arp); /* arp */
